...
For example: `ojp-server/target/ojp-server-0.1.0-beta-shaded.jar`
...
```bash
java -jar ojp-server/target/ojp-server-0.1.0-beta-shaded.jar
```
...
java -Dojp.server.port=8080 \
     ...
     -jar ojp-server/target/ojp-server-0.1.0-beta-shaded.jar
```
...
```bash
nohup java -jar ojp-server/target/ojp-server-0.1.0-beta-shaded.jar > ojp-server.log 2>&1 &
```
...
```bash
java -Xmx2g -jar ojp-server/target/ojp-server-0.1.0-beta-shaded.jar
```
