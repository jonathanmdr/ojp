...
public class StatementServiceImpl extends StatementServiceGrpc.StatementServiceImplBase {
    ...
    public void executeUpdate(StatementRequest request, StreamObserver<OpResult> responseObserver) {
        ...
        if (request.getSql().contains("insert into users (email,username) values (?,?)")) {
            int stop = 0;
        }
        ...
        try {
            dto = sessionConnection(request.getSession(), this.isAddBatchOperation(request));//TODO revieew || true might need to check for generated keys
