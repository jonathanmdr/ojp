...
public class Db2MultipleTypesIntegrationTest {
    ...
    public void typesCoverageTestSuccessful(String driverClass, String url, String user, String pwd) throws SQLException, ClassNotFoundException, ParseException {
        ...
        TestDBUtils.createMultiTypeTestTable(conn, "db2_multi_types_test", TestDBUtils.SqlSyntax.DB2);

        java.sql.PreparedStatement psInsert = conn.prepareStatement(
                "insert into db2_multi_types_test (val_int, val_varchar, val_double_precision, val_bigint, val_tinyint, " +
        ...
        java.sql.PreparedStatement psSelect = conn.prepareStatement("select * from db2_multi_types_test where val_int = ?");
        psSelect.setInt(1, 1);
    ...
    public void testDb2NumericTypes(String driverClass, String url, String user, String pwd) throws SQLException, ClassNotFoundException {
        ...
        try {
            TestDBUtils.executeUpdate(conn, "DROP TABLE test_db2_numbers");
        } catch (Exception e) {
            // Ignore if table doesn't exist
        }

        TestDBUtils.executeUpdate(conn, 
            "CREATE TABLE test_db2_numbers (" +
            "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " +
        ...
        java.sql.PreparedStatement psInsert = conn.prepareStatement(
            "INSERT INTO test_db2_numbers (smallint_col, integer_col, bigint_col, decimal_col, numeric_col, real_col, double_col) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        ...
        java.sql.PreparedStatement psSelect = conn.prepareStatement("SELECT * FROM test_db2_numbers WHERE id = 1");
        ResultSet resultSet = psSelect.executeQuery();
        
        Assert.assertTrue(resultSet.next());
        Assert.assertEquals(32767, resultSet.getShort("smallint_col"));
